...
async function Save_Building_Result() {
  // // 将球体和贝塞尔曲线控制点的坐标保存到 Building_Result 中
  // result.value = []
  ...
  //   })
  // }
  // for (let i = 0; i < Bezier_Points_Pos.value.length; i++) {
  //   const point = Bezier_Points_Pos.value[i]
  //   result.value.push({
  ...
  //   })
  // }
  // Send_Building_Result.value.result = result.value
  // Send_Building_Result.value.user_name = login_data.value[0].username
  // const response = await api.post('/api/Save_BuildingResult_AsJson', 
  //   Send_Building_Result.value
  // )
  // result.value = []
  // Send_Building_Result.value.result = []
  // Send_Building_Result.value.user_name = ''
  // response.status === 201 ? console.log('保存成功') : console.error('保存失败')
}
